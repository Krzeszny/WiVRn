        ...
        right_hand_interaction(1, &hmd, this)
{
	...
	if (use_steamvr_lh && steamvr_lh_create_devices(&lh_devices) == XRT_SUCCESS)
	{
...
xrt_device_name wivrn_session::lh_device_activity_check(size_t idx, xrt_device * fallback_xdev, int32_t * role, xrt_device ** ht_role, bool * dirty)
{
...
void wivrn_session::operator()(from_headset::trackings && tracking)
{
	...
	auto lh_left = lh_device_activity_check(0, &left_controller, &roles.left, &static_roles.hand_tracking.left, &dirty);
	auto lh_right = lh_device_activity_check(1, &right_controller, &roles.right, &static_roles.hand_tracking.right, &dirty);
