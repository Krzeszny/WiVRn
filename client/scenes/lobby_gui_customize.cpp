...
void scenes::lobby::use_environment(const environment_model & model)
{
	future_environment = utils::async<std::pair<std::string, std::shared_ptr<entt::registry>>, float>(
	        [this](utils::async_token<std::pair<std::string, std::shared_ptr<entt::registry>>, float> token, std::filesystem::path path) {
...
scenes::lobby::environment_item_action scenes::lobby::environment_item(environment_model & model, bool download_screenshot)
{
	...
	ImVec2 selectable_size{
	        width,
	        screenshot_size.y + 2 * style.FramePadding.y +          // Screenshot size includeing padding
	...
	bool selected = application::get_config().environment_model == model.local_gltf_path;
	...
	{
		if (local)
			imgui_ctx->tooltip(_("Use this model"));
		else
			imgui_ctx->tooltip(_("Download this model"));
	}
	imgui_ctx->vibrate_on_hover();
	auto end_cursor = ImGui::GetCursorScreenPos();

	// Remove the height of the progress bar if a transfer is in progress
	float text_clip_height = selectable_size.y;
	if (auto handle = try_get_download_handle(model.gltf_url); handle and handle->get_state() == libcurl::state::transferring)
		text_clip_height -= ImGui::GetFontSize() + 2 * style.FramePadding.y;

	// Display the screenshot
	ImGui::SetCursorScreenPos(backup_cursor + style.FramePadding);
	ImGui::Image(screenshot, screenshot_size);

	ImGui::PushClipRect(backup_cursor + style.FramePadding, backup_cursor + ImVec2(selectable_size.x, text_clip_height) - style.FramePadding, true);
	float indent = style.FramePadding.x + screenshot_size.x + 2 * style.ImageBorderSize + style.ItemSpacing.x;
	...
	ImVec2 text_pos = backup_cursor + ImVec2(indent, style.FramePadding.y);
	float text_height = 0;
	...
	// if (text_height > selectable_size.y - 2 * style.FramePadding.y)
	// {
	// 	// TODO use ICON_FA_CHEVRON_DOWN and ICON_FA_CHEVRON_UP to expand/collapse the text
	// }
	...
	{
		auto delete_size = ImGui::CalcTextSize(ICON_FA_TRASH) + style.FramePadding * 2.0f;
		ImGui::SetCursorScreenPos(ImVec2(backup_cursor.x + width - delete_size.x - style.WindowPadding.x, backup_cursor.y));
	...
	else if (auto handle = try_get_download_handle(model.gltf_url))
	{
		switch (handle->get_state())
		{
			case libcurl::state::transferring: {
				...
				ImGui::SetCursorScreenPos(ImVec2(backup_cursor.x, backup_cursor.y + selectable_size.y - cancel_size.y));

				float progress_width = width - cancel_size.x - style.ItemSpacing.x;
...
void scenes::lobby::gui_customize()
{
	...
	auto download = _("Download more environments");
	auto download_size = ImGui::CalcTextSize(download.c_str());

	ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, {0, 0});
	ImGui::BeginChild("environment list", ImGui::GetWindowSize() - ImGui::GetCursorPos() - ImVec2(0, download_size.y + 80));

	ImGui::BeginDisabled(system.passthrough_supported() == xr::passthrough_type::none);
	if (ImGui::RadioButton(_S("Passthrough"), config.passthrough_enabled))
	{
		...
		switch (index_transfer ? index_transfer->get_state() : libcurl::state::reset)
		{
			...
			case libcurl::state::reset:
				if (downloadable_environment_list_status == "")
					...
					CenterTextHV(downloadable_environment_list_status); // TODO retry button
		}
